...
from .rnn_reader import RnnDocReader4Classify, RnnDocReader4BinaryClassify

logger = logging.getLogger(__name__)

class DocReader4BinaryClassify(object):
    ...
    def __init__(self, args, word_dict, char_dict,feature_dict,
        ...
        self.agree_lambda = 1.0 #2.0
        assert args.positive_weight < 1.0 and args.positive_weight > 0.0
        self.agree_weight = torch.Tensor([1-args.positive_weight, args.positive_weight]).cuda()#torch.Tensor([0.33, 0.67]).cuda()
        
        # Building network. If normalize if false, scores are not normalized
        # 0-1 per paragraph (no softmax).
        if args.model_type == 'rnn':
            self.network = RnnDocReader4BinaryClassify(args, normalize)
        else:
    ...
    def update(self, ex):
        ...
        # Run forward
        score_s, _ = self.network(*inputs)
        # Compute loss and accuracies
        
        #print(target_s.shape, target_s)
        #print(target_e.shape, target_e)
        #print(score_s.shape, score_s)
        #print(score_e.shape, score_e)
       
        #rela_mask = (target_s==1)
        #target_e = mas
        #print(rela_mask.shape) [32]
        #print(target_e.shape)
        #print(score
        rela_loss = F.nll_loss(score_s, target_s, weight=self.agree_weight)
        #agree_loss = F.nll_loss(score_e, target_e, weight=self.agree_weight, ignore_index = -1)
        #print(agree_loss.shape)
        loss =  rela_loss #+ self.agree_lambda * agree_loss#.masked_select(rela_mask)
    ...
    def predict(self, ex, candidates=None, top_n=1, async_pool=None):
        ...
        loss = F.nll_loss(score_s, target_s) 
        ...
        if candidates:
            ...
            if async_pool:
                ...
                return self.decode_s(*args), loss.item()

    @staticmethod
    def decode_s(score_s, score_e, top_n=1, max_len=None):
    ...
    def load(filename, new_args=None, normalize=True):
        ...
        return DocReader4BinaryClassify(args, word_dict, char_dict, feature_dict, state_dict, normalize)
...
class DocReader4Classify(object):
    """High level model that handles intializing the underlying network
    architecture, saving, updating examples, and predicting examples.
    """
    ...
    def __init__(self, args, word_dict, char_dict,feature_dict,
        ...
        self.agree_lambda = 1.0 #2.0
        self.agree_weight = torch.Tensor([0.5, 0.5]).cuda()#torch.Tensor([0.33, 0.67]).cuda()
        
        # Building network. If normalize if false, scores are not normalized
        # 0-1 per paragraph (no softmax).
        if args.model_type == 'rnn':
            self.network = RnnDocReader4Classify(args, normalize)
        else:
            raise RuntimeError('Unsupported model: %s' % args.model_type)
    ...
    def update(self, ex):
        ...
        #print(target_e.shape)
        #print(score
        rela_loss = F.nll_loss(score_s, target_s)
        agree_loss = F.nll_loss(score_e, target_e, weight=self.agree_weight, ignore_index = -1)
        #print(agree_loss.shape)
        loss =  rela_loss + self.agree_lambda * agree_loss#.masked_select(rela_mask)
    ...
    def predict(self, ex, candidates=None, top_n=1, async_pool=None):
        ...
        loss = F.nll_loss(score_s, target_s) + F.nll_loss(score_e, target_e, ignore_index = -1)
