...
# Network
# ------------------------------------------------------------------------------
class RnnDocReader4BinaryClassify(nn.Module):
    RNN_TYPES = {'lstm': nn.LSTM, 'gru': nn.GRU, 'rnn': nn.RNN}

    def __init__(self, args, normalize=True):
        super(RnnDocReader4BinaryClassify, self).__init__()
        # Store config
        self.args = args
        args.char_emb = 20
    ...
    def forward(self, x1, x1_c, x1_mask, x2, x2_c, x2_mask):
        ...
        #print(g1.shape) 32 * 120 * 256
        #g1.data.masked_fill_(x1_mask.data, -2e20)
        g1_mean = torch.mean(g1, dim=1)
        #print(g1_mean.shape) # 32*256
        

        #print(torch.cat([c_fusion, g1], dim=2).shape) #30,120,1280
        #print(self.start( torch.cat([c_fusion, g1], dim=2) ).shape) #30,120,1
        
        related_logits = self.related(g1_mean)
        softmax_related = F.softmax(related_logits, dim = 1)
        #print(softmax_related)
        
        #agreed_logits = self.agreed(g1_mean)
        #softmax_agreed = F.softmax(agreed_logits, dim = 1)
       
        
        if self.training:
            related_scores = torch.log(softmax_related + 1e-20)
            #agreed_scores = torch.log(softmax_agreed + 1e-20)
        else:
            related_scores = softmax_related
            #agreed_scores = softmax_agreed
        #print(related_logits.shape) 32, 2
        return related_scores, None#, agreed_scores
        '''
...
class RnnDocReader4Classify(nn.Module):
    RNN_TYPES = {'lstm': nn.LSTM, 'gru': nn.GRU, 'rnn': nn.RNN}

    def __init__(self, args, normalize=True):
        super(RnnDocReader4Classify, self).__init__()
        # Store config
        self.args = args
        args.char_emb = 20
    ...
    def forward(self, x1, x1_c, x1_mask, x2, x2_c, x2_mask):
        ...
        if self.training:
            ...
            agreed_scores = softmax_agreed
        #print(related_logits.shape) 32, 2
        return related_scores, agreed_scores
        '''
        # input: bxnx(64*4*4+128*2)
        start_logits = self.start( torch.cat([c_fusion, g1], dim=2) ).squeeze(2)
        ...
        #print(start_scores.shape, end_scores.shape) [32, 120]
